...
app.factory('dataService',['$http',function($http){
    ...
    search: function(name){
        return $http.get("/api/search", {params:{search:name}})
        .then(function (res){
            return res.data;
        })
    }
...
app.factory('posts',  ['$http', 'userMessages', 'auth', function($http, userMessages, auth){
    ...
    o.create = function(post, category) {
        ...
        headers: {Authorization: 'Bearer '+auth.getToken()}
            ...
            o.posts.push(data);
            userMessages.setMessage("Your Question was Added Successfully");
        });
    };
    o.edit = function(updateData, post, category) {
        ...
        headers: {Authorization: 'Bearer '+auth.getToken()}
           ...
           o.posts.push(data);
           userMessages.setMessage("Your Question was Edited Successfully");
        });
    };
   ...
   o.downvote = function(post) {
    ...
    o.deleteOne = function(id) {
        ...
        headers: {Authorization: 'Bearer '+auth.getToken()}
            ...
            userMessages.setMessage("Your Question was Deleted");
            return res.data;
        });
    }
    o.close = function(post) {
        ...
        headers: {Authorization: 'Bearer '+auth.getToken()}
            ...
            post.active = false;
            userMessages.setMessage("The Question '" + post.title + "' has been closed.");
        });
    };
    o.open = function(post) {
        ...
        headers: {Authorization: 'Bearer '+auth.getToken()}
            ...
            post.active = true;
            userMessages.setMessage("The Question '" + post.title + "' has been reopened.");
        });
    };
...
// USERS service
app.factory('users',  ['$http', 'userMessages', 'auth','$q', function($http, userMessages, auth,$q){
    // return {
    //     allUsers : function(){
    //         var deferred = $q.defer();
    //      $http.get('/api/allUsers').success(function(res){
    //     var user = res.data;
...
app.factory('auth', ['$http','$window', '$timeout', 'userMessages', function($http, $window, $timeout, userMessages){
    ...
    auth.getToken = function (){
        console.log($window.localStorage['forum-token']);
