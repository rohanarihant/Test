...
router.param('post', function(req, res, next, id) {
  ...
  query.exec(function (err, post){
    ...
    if (!post) { return next(new Error('can\'t find Question')); }
...
// Comment Routes ----------------------------------------------------------------- 
    ...
    router.post('/api/comments/:post/comments', auth, function(req, res, next) { 
        ...
        if(req.post.active && comment.body.length > 1){
            ...
            var err = ("Failed. The Question Is Inactive.")
            res.send(err);
            return next(err);
        }
    });
...
// User Registration Routes -----------------------------------------------------------------
    ...
    router.post('/api/allUsers',function(req,res,next){
        User.count({userrole:'User'},function(err,count){
